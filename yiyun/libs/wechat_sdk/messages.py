...
class TicketMessage(WechatMessage):

    def __init__(self, message):
        self.ticket = message.pop('ComponentVerifyTicket', '')
        super(TicketMessage, self).__init__(message)


@handle_for_type('unauthorized')
class UnauthorizedMessage(WechatMessage):

    def __init__(self, message):
        self.authorizer_appid = message.pop('AuthorizerAppid', '')
        super(UnauthorizedMessage, self).__init__(message)


@handle_for_type('authorized')
class AuthorizedMessage(WechatMessage):

    def __init__(self, message):
        self.authorizer_appid = message.pop('AuthorizerAppid', '')
        self.authorization_code = message.pop('AuthorizationCode', '')
        self.expired_time = message.pop('AuthorizationCodeExpiredTime', '')
        super(AuthorizedMessage, self).__init__(message)


@handle_for_type('updateauthorized')
class UpdateAuthorizedMessage(WechatMessage):

    def __init__(self, message):
        self.authorizer_appid = message.pop('AuthorizerAppid', '')
        self.authorization_code = message.pop('AuthorizationCode', '')
        self.expired_time = message.pop('AuthorizationCodeExpiredTime', '')
        super(UpdateAuthorizedMessage, self).__init__(message)

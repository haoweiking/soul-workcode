...
class WechatBasic(WechatBase):
    ...
    """
    def __init__(self, token=None, appid=None, appsecret=None, encrypt_mode="normal", encoding_aes_key=None, partnerid=None,
        ...
        if conf is not None:
            ...
            self.__conf = WechatConf(
                ...
                checkssl=checkssl,
                access_token_getfunc=access_token_getfunc
            )
    ...
    def parse_data(self, data, msg_signature=None, timestamp=None, nonce=None):
        ...
        if "MsgType" in result:
            result['type'] = result.pop('MsgType').lower()
            
        elif "InfoType" in result:
            result['type'] = result.pop('InfoType').lower()
